...
sudo tail -n2000 $IS_MA__WEBLOG_PATH | awk '{ print $NF }' | cut -c 12-16 | uniq --count | grep $one_min_ago > /tmp/last_min_hits.txt
...
sudo tail -n2000 $IS_MA__WEBLOG_PATH | grep -e "T$hour_min:[0-9][0-9]\-" | awk '{ print $8 }' | sort | uniq --count | sort -nr | head -n1 > /tmp/top_ip_hits.txt
