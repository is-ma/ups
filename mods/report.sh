...
# include
source $UFW_POLICE_PATH/mods/function.save_ip_in_any_bag.sh

# get last minute number of hits and the exact hour:minute 
one_min_ago=$(date --date='1 minute ago' '+%H:%M')
sudo tail -n2000 $WEBLOG_PATH | cut -d: -f2,3 | uniq --count | grep $one_min_ago > /tmp/last_min_hits.txt
...
sudo tail -n2000 $WEBLOG_PATH | grep -e ":$hour_min:[0-9][0-9] " | cut -d' ' -f1 | sort | uniq --count | sort -nr | head -n1 > /tmp/top_ip_hits.txt
